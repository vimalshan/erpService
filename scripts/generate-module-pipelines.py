"""
Generate one azure-pipelines.yml per module at src/Services/<module>/azure-pipelines.yml
Service registry is copied exactly from the existing .github/workflows/build-*.yml files.
"""
...
MODULES = {
    ...
    "auditServices": {
        ...
        ("documents-service",     "src/Services/auditServices/documentsapiServices",     "Dockerfile"),
        ...
        ("overview-service",      "src/Services/auditServices/overviewapiServices",      "Dockerfile"),
...
# ── Template builder ──────────────────────────────────────────────────────────
def make_pipeline(module_name, module_path, services):
    ...
    return f"""\
# Azure DevOps Pipeline - {module_name}
# Builds and pushes all {len(services)} service images to GHCR.
# Registry: ghcr.io/$(githubOwner)/erp/<service-name>
#
# Setup required (once):
#   Project Settings -> Service connections -> New -> Docker Registry
#   Name: ghcr-service-connection  Type: Other  Registry: ghcr.io
#   Username: <github-username>   Password: <GitHub PAT with packages:write>

name: Build & Push {module_name}
...
{matrix_block}
        ...
        steps:
          ...
          - task: Docker@2
            ...
            condition: or(eq('${{{{ parameters.service }}}}', 'all'), eq('${{{{ parameters.service }}}}', variables['imageName']))
            inputs:
              command: build
...
{matrix_block}
        ...
        steps:
          - script: |
              ...
              docker logs "$CNAME"
              docker rm -f "$CNAME"
            displayName: Smoke test $(imageName)
            condition: or(eq('${{{{ parameters.service }}}}', 'all'), eq('${{{{ parameters.service }}}}', variables['imageName']))
"""


# ── Generate all module pipelines ─────────────────────────────────────────────
count = 0
for module_name, cfg in MODULES.items():
    out_path = REPO / cfg["path"] / "azure-pipelines.yml"
    out_path.parent.mkdir(parents=True, exist_ok=True)
    content = make_pipeline(module_name, cfg["path"], cfg["services"])
    out_path.write_text(content, encoding="utf-8")
    print(f"  [{len(cfg['services']):2d} svcs]  {out_path.relative_to(REPO)}")
    count += 1

print(f"\nDone. Generated {count} module-level azure-pipelines.yml files.")
