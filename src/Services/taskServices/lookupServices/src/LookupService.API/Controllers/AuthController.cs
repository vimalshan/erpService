...
public class AuthController(IConfiguration configuration) : ControllerBase
{
    ...
    private TokenResponse GenerateJwtToken(string username)
    {
        var key = new SymmetricSecurityKey(Encoding.UTF8.GetBytes(
            configuration["Jwt:Key"] ?? "LookupService-Super-Secret-Key-For-JWT-Authentication-2024!"));
        ...
        var token = new JwtSecurityToken(
            issuer: configuration["Jwt:Issuer"] ?? "LookupService",
            audience: configuration["Jwt:Audience"] ?? "LookupService",
            claims: claims,
            expires: expiry,
