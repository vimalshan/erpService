...
-- Purpose: Cash Functions, Stored Procedures & Triggers
...
BEGIN
    ...
    BEGIN TRY
        ...
        -- Get ledger balance
        SET @LedgerBalance = dbo.fn_GetBankBalance(@p_BankAccountID, CAST(@p_ReconciliationDate AS DATETIME2(3)));
        
        -- Get uncleared cheques
        SET @UnclearedCheques = dbo.fn_GetUnclearedChequesTotal(@p_BankAccountID, CAST(@p_ReconciliationDate AS DATETIME2(3)));
    ...
    BEGIN TRY
        ...
        IF EXISTS (
            ...
            WHERE BANK_ACCOUNT_ID = @p_BankAccountID
              ...
              AND CHEQUE_STATUS NOT IN ('X', 'C')  -- Not cancelled or cleared
        )
        BEGIN
...
-- Procedure: Mark Cheque as Bounced
...
PRINT 'CashManagement Module Procedures, Functions & Triggers created successfully.';
GO

-- Verify function creation
IF OBJECT_ID('dbo.fn_GetCashInHand', 'FN') IS NOT NULL
    PRINT 'Function fn_GetCashInHand: OK'
ELSE
    PRINT 'Function fn_GetCashInHand: FAILED'
GO

IF OBJECT_ID('dbo.fn_GetBankBalance', 'FN') IS NOT NULL
    PRINT 'Function fn_GetBankBalance: OK'
ELSE
    PRINT 'Function fn_GetBankBalance: FAILED'
GO

IF OBJECT_ID('dbo.fn_GetUnclearedChequesTotal', 'FN') IS NOT NULL
    PRINT 'Function fn_GetUnclearedChequesTotal: OK'
ELSE
    PRINT 'Function fn_GetUnclearedChequesTotal: FAILED'
GO

-- Verify procedure creation
IF OBJECT_ID('dbo.usp_RecordCashReceipt', 'P') IS NOT NULL
    PRINT 'Procedure usp_RecordCashReceipt: OK'
ELSE
    PRINT 'Procedure usp_RecordCashReceipt: FAILED'
GO

IF OBJECT_ID('dbo.usp_RecordCashDisbursement', 'P') IS NOT NULL
    PRINT 'Procedure usp_RecordCashDisbursement: OK'
ELSE
    PRINT 'Procedure usp_RecordCashDisbursement: FAILED'
GO

IF OBJECT_ID('dbo.usp_PerformBankReconciliation', 'P') IS NOT NULL
    PRINT 'Procedure usp_PerformBankReconciliation: OK'
ELSE
    PRINT 'Procedure usp_PerformBankReconciliation: FAILED'
GO

IF OBJECT_ID('dbo.usp_IssueCheque', 'P') IS NOT NULL
    PRINT 'Procedure usp_IssueCheque: OK'
ELSE
    PRINT 'Procedure usp_IssueCheque: FAILED'
GO

IF OBJECT_ID('dbo.usp_MarkChequeBounced', 'P') IS NOT NULL
    PRINT 'Procedure usp_MarkChequeBounced: OK'
ELSE
    PRINT 'Procedure usp_MarkChequeBounced: FAILED'
GO
