...
public class AlertNotificationConsumer : BackgroundService
{
    ...
    private const string QueueName = "alerts-notifications-queue";
    private const string RoutingKey = "alert.notification.#";

    public AlertNotificationConsumer(IConfiguration configuration, ILogger<AlertNotificationConsumer> logger)
    {
    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        ...
        {
            _logger.LogWarning(ex, "AlertNotificationConsumer could not connect to RabbitMQ. Consumer will not run.");
        }
    }
    ...
    private async Task StartConsumingAsync(CancellationToken stoppingToken)
    {
        ...
        consumer.ReceivedAsync += async (_, ea) =>
        {
            try
            {
                ...
                _logger.LogInformation("Received alert notification message: {Message}", message);
            ...
            {
                _logger.LogError(ex, "Error processing alert notification message");
        ...
        _logger.LogInformation("AlertNotificationConsumer started listening on queue: {Queue}", QueueName);
