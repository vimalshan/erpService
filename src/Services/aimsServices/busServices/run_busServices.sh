...
echo "Stopping and removing existing container: erp-bus-service"
docker rm -f erp-bus-service 2>/dev/null || true

# Rebuild image
echo "Building image: bus-service (lowercase required for Docker)"
docker build -t bus-service:latest -f Dockerfile .
...
docker run -d \
  -p 5012:80 \
  --name erp-bus-service \
  --network erp-network \
  -e ASPNETCORE_ENVIRONMENT=Development \
  -e ASPNETCORE_URLS=http://+:80 \
  -e ConnectionStrings__BusDb="Server=sqlserver;Database=BUSDB;User Id=sa;Password=YourStr0ng!Passw0rd;TrustServerCertificate=true;" \
  -e RabbitMQ__Host=rabbitmq \
  -e RabbitMQ__Port=5672 \
  -e RabbitMQ__Username=guest \
  -e RabbitMQ__Password=guest \
  -e RabbitMQ__BusRegisteredQueue=bus.registered \
  -e RabbitMQ__EmployeeAssignedQueue=bus.employee.assigned \
  -e Jwt__Key=BusServices-SuperSecretKey-Min32Chars!! \
  -e Jwt__Issuer=BusServices.API \
  -e Jwt__Audience=BusServices.API \
  bus-service:latest
...
echo "Testing service at http://localhost:5012/health"
if command -v curl &> /dev/null; then
    MAX_RETRIES=10
    RETRY_INTERVAL=5
    for i in $(seq 1 $MAX_RETRIES); do
        echo "Attempt $i/$MAX_RETRIES..."
        HTTP_STATUS=$(curl -s -o /tmp/health_response.json -w "%{http_code}" http://localhost:5012/health)
        ...
        if [ "$i" -eq "$MAX_RETRIES" ]; then
            ...
            docker logs --tail 30 erp-bus-service
        else
