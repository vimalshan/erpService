...
echo "Stopping and removing existing container: erp-reference-service"
docker rm -f erp-reference-service 2>/dev/null || true

# Rebuild image
echo "Building image: reference-service"
docker build -t reference-service:latest -f Dockerfile .

# Ensure the shared network exists (created by docker-compose.shared.yml)
docker network create erp-network 2>/dev/null || true

# Run container
echo "Starting container on port 5017"
docker run -d \
  -p 5017:80 \
  --name erp-reference-service \
  --network erp-network \
  -e ASPNETCORE_ENVIRONMENT=Production \
  -e ASPNETCORE_URLS=http://+:80 \
  -e ConnectionStrings__DefaultConnection="Server=sqlserver;Database=REFERENCEDB;User Id=sa;Password=YourStr0ng!Passw0rd;TrustServerCertificate=true;" \
  -e RabbitMQ__Host=rabbitmq \
  -e RabbitMQ__Port=5672 \
  -e RabbitMQ__Username=guest \
  -e RabbitMQ__Password=guest \
  -e RabbitMQ__VirtualHost=/ \
  -e RabbitMQ__ExchangeName=reference.events \
  -e RabbitMQ__LovTypeQueue=reference.lovtype.updates \
  -e Jwt__Key=CHANGE_ME_TO_A_STRONG_256BIT_SECRET_KEY_IN_PRODUCTION \
  -e Jwt__Issuer=reference-service \
  -e Jwt__Audience=reference-service-api \
  reference-service:latest
...
# Check logs
echo "Container logs:"
docker logs erp-reference-service
...
echo "Testing service at http://localhost:5017/health"
if command -v curl &> /dev/null; then
    MAX_RETRIES=10
    RETRY_INTERVAL=5
    for i in $(seq 1 $MAX_RETRIES); do
        echo "Attempt $i/$MAX_RETRIES..."
        HTTP_STATUS=$(curl -s -o /tmp/health_response.json -w "%{http_code}" http://localhost:5017/health)
        ...
        if [ "$i" -eq "$MAX_RETRIES" ]; then
            ...
            docker logs --tail 30 erp-reference-service
        else
