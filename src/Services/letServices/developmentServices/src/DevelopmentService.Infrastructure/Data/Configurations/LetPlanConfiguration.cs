...
public class LetPlanConfiguration : IEntityTypeConfiguration<LetPlan>
{
    public void Configure(EntityTypeBuilder<LetPlan> builder)
    {
        builder.ToTable("DD_LETPLAN");
        ...
        builder.HasIndex(x => x.UserId).HasDatabaseName("IDX_DD_LETPLAN_USERID");
        builder.HasIndex(x => x.Priority).HasDatabaseName("IDX_DD_LETPLAN_PRIORITY");
        builder.HasIndex(x => x.AppStatus).HasDatabaseName("IDX_DD_LETPLAN_STATUS");
