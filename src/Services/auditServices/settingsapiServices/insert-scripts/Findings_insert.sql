...
([FindingId], [FindingNumber], [AuditId], [SiteId], [Title], [Description], [FindingType], [Severity], [FindingStatusId], [FindingCategoryId], [IdentifiedDate], [DueDate], [IsActive], [CreatedDate], [ModifiedDate], [CreatedBy], [ModifiedBy], [IdentifiedBy], [AssignedTo], [Evidence], [RootCause], [CorrectiveAction], [PreventiveAction], [CompletionDate])
SELECT
    FindingId,
    FindingNumber,
    AuditId,
    ...
    FindingDescription,
    CASE
        WHEN CategoryId = 3 THEN 'Observation'
        WHEN CategoryId = 4 THEN 'OFI'
        ELSE 'NC'
    END,
    CASE
        WHEN Severity IN ('Low', 'Medium') THEN 'Minor'
        ELSE Severity
    END,
    StatusId,
    CategoryId,
    ...
    RaisedBy,
    AssignedTo,
    CASE WHEN ClauseReference IS NULL THEN NULL ELSE CONCAT('Clause: ', ClauseReference) END,
    RootCause,
    CorrectiveAction,
    PreventiveAction,
    NULL
...
) AS v(FindingId, AuditId, SiteId, FindingNumber, FindingTitle, FindingDescription, CategoryId, StatusId, Severity, ClauseReference, RaisedDate, DueDate, RaisedBy, AssignedTo, RootCause, CorrectiveAction, PreventiveAction, IsActive, CreatedDate, ModifiedDate, CreatedBy, ModifiedBy);
...
LEFT JOIN [dbo].[Findings] f ON fs.FindingStatusId = f.FindingStatusId
GROUP BY fs.FindingStatusId, fs.StatusName, fs.DisplayOrder
...
SELECT 
    ...
    COUNT(CASE WHEN f.FindingStatusId = 5 THEN 1 END) as ClosedFindings,
    COUNT(CASE WHEN f.FindingStatusId IN (1,2,3) THEN 1 END) as OpenFindings,
    COUNT(CASE WHEN f.FindingStatusId = 6 THEN 1 END) as OverdueFindings
...
WHERE f.FindingStatusId = 6 OR (f.DueDate < GETDATE() AND f.FindingStatusId NOT IN (5))
...
LEFT JOIN [dbo].[Findings] f ON fc.FindingCategoryId = f.FindingCategoryId
GROUP BY fc.FindingCategoryId, fc.CategoryName
ORDER BY Count DESC;