...
([NotificationId], [CategoryId], [Title], [Message], [ActionUrl], [Priority], [Status], [ExpiryDate], [CreatedDate], [ModifiedDate], [CreatedBy], [ModifiedBy], [IsActive], [ReadBy], [TargetAudience])
SELECT
    NotificationId,
    CategoryId,
    Title,
    ...
    ActionUrl,
    Priority,
    CASE WHEN IsRead = 1 THEN 'Read' ELSE 'Active' END,
    ExpiryDate,
    CreatedDate,
    COALESCE(ReadDate, CreatedDate),
    CreatedBy,
    CreatedBy,
    1,
    CASE WHEN IsRead = 1 THEN CONCAT('[', UserId, ']') ELSE NULL END,
    'User'
FROM (VALUES
...
) AS v(NotificationId, UserId, CategoryId, Title, Message, ActionUrl, IsRead, Priority, ExpiryDate, CreatedDate, ReadDate, CreatedBy);
...
SELECT 
    ...
    COUNT(CASE WHEN n.Status = 'Read' THEN 1 END) as ReadCount,
    COUNT(CASE WHEN n.Status <> 'Read' THEN 1 END) as ActiveCount
...
WHERE n.ExpiryDate < GETDATE() AND n.Status <> 'Read'
