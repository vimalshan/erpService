...
([TrainingId], [TrainingName], [TrainingCode], [TrainingType], [Description], [Category], [Duration], [DueDate], [Prerequisites], [LearningObjectives], [IsActive], [CreatedDate], [ModifiedDate], [CreatedBy], [ModifiedBy])
SELECT
    TrainingId,
    TrainingName,
    CONCAT('TRN-', RIGHT('000' + CAST(TrainingId AS VARCHAR(3)), 3)),
    CASE WHEN DeliveryMethod = 'Blended' THEN 'Hybrid' ELSE DeliveryMethod END,
    [Description],
    Category,
    Duration,
    NULL,
...
) AS v(TrainingId, TrainingName, [Description], Category, Duration, DeliveryMethod, Prerequisites, LearningObjectives, IsActive, CreatedDate, UpdatedDate, CreatedBy, UpdatedBy);
...
SELECT 
    ...
    COUNT(CASE WHEN TrainingType = 'Online' THEN 1 END) as OnlineTrainings,
    COUNT(CASE WHEN TrainingType = 'Classroom' THEN 1 END) as ClassroomTrainings,
    COUNT(CASE WHEN TrainingType = 'Hybrid' THEN 1 END) as HybridTrainings
FROM [dbo].[Trainings] 
WHERE IsActive = 1
GROUP BY Category
