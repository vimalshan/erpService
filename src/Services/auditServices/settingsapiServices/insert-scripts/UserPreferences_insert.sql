...
([UserPreferenceId], [UserId], [PreferenceKey], [PreferenceValue], [Category], [PreferenceType], [IsActive], [CreatedDate], [ModifiedDate], [CreatedBy], [ModifiedBy])
VALUES
...
SELECT 
    ...
    COUNT(up.UserPreferenceId) as TotalPreferences,
    COUNT(CASE WHEN up.Category = 'Notifications' THEN 1 END) as NotificationPreferences,
    MAX(CASE WHEN up.PreferenceKey = 'EmailNotifications' THEN up.PreferenceValue END) as EmailEnabled,
    MAX(CASE WHEN up.PreferenceKey = 'EmailFrequency' THEN up.PreferenceValue END) as EmailFrequency
FROM [dbo].[Users] u
LEFT JOIN [dbo].[UserPreferences] up ON u.UserId = up.UserId
GROUP BY u.UserId, u.FirstName, u.LastName
HAVING COUNT(up.UserPreferenceId) > 0
ORDER BY TotalPreferences DESC;

-- Show language and timezone distribution
SELECT 
    MAX(CASE WHEN up.PreferenceKey = 'Language' THEN up.PreferenceValue END) as Language,
    MAX(CASE WHEN up.PreferenceKey = 'TimeZone' THEN up.PreferenceValue END) as TimeZone,
    COUNT(DISTINCT up.UserId) as UserCount
FROM [dbo].[UserPreferences] up
WHERE up.PreferenceKey IN ('Language', 'TimeZone')
GROUP BY up.UserId
HAVING MAX(CASE WHEN up.PreferenceKey = 'Language' THEN up.PreferenceValue END) IS NOT NULL
...
WHERE up.PreferenceKey = 'EmailFrequency'
