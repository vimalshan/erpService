...
([InvoiceId], [InvoiceNumber], [CompanyId], [ContractId], [InvoiceDate], [DueDate], [PlannedPaymentDate], [PaidDate], [Amount], [TaxAmount], [TotalAmount], [Currency], [Status], [PaymentMethod], [Description], [Notes], [IsActive], [CreatedDate], [ModifiedDate], [CreatedBy], [ModifiedBy])
SELECT
    InvoiceId,
    InvoiceNumber,
    CompanyId,
    ...
    TotalAmount,
    Currency,
    CASE WHEN Status = 'Outstanding' THEN 'Overdue' ELSE Status END,
    PaymentMethod,
    Description,
    BillingAddress,
    IsActive,
...
) AS v(InvoiceId, InvoiceNumber, CompanyId, AuditId, ServiceId, InvoiceDate, DueDate, Amount, Currency, TaxAmount, TotalAmount, Status, PaymentDate, PaymentMethod, Description, BillingAddress, IsActive, CreatedDate, ModifiedDate, CreatedBy, ModifiedBy);
