...
([UserTrainingId], [UserId], [TrainingId], [EnrollmentDate], [StartDate], [CompletionDate], [Status], [Score], [CertificateNumber], [CertificateExpiryDate], [Notes], [CreatedDate], [ModifiedDate], [CreatedBy], [ModifiedBy], [Progress], [CertificateIssued])
SELECT
    UserTrainingId,
    UserId,
    TrainingId,
...
) AS v(UserTrainingId, UserId, TrainingId, EnrollmentDate, StartDate, CompletionDate, Status, Score, CertificateNumber, CertificateExpiryDate, Notes, CreatedDate, ModifiedDate, CreatedBy, ModifiedBy);
...
SELECT 
    CASE 
        WHEN u.UserId <= 20 THEN 'DNV Staff'
        ELSE 'Customer'
    END as UserType,
...
GROUP BY CASE WHEN u.UserId <= 20 THEN 'DNV Staff' ELSE 'Customer' END
...
SELECT 
    u.FirstName + ' ' + u.LastName as UserName,
    t.TrainingName,
    ut.CertificateNumber,
    ut.CertificateExpiryDate,
    DATEDIFF(day, GETDATE(), ut.CertificateExpiryDate) as DaysUntilExpiry,
    CASE 
        WHEN ut.CertificateExpiryDate < GETDATE() THEN 'Expired'
        WHEN DATEDIFF(day, GETDATE(), ut.CertificateExpiryDate) <= 90 THEN 'Expiring Soon'
        WHEN DATEDIFF(day, GETDATE(), ut.CertificateExpiryDate) <= 180 THEN 'Renewal Due'
        ELSE 'Current'
    END as ExpiryStatus
FROM [dbo].[Users] u
INNER JOIN [dbo].[UserTrainings] ut ON u.UserId = ut.UserId
INNER JOIN [dbo].[Trainings] t ON ut.TrainingId = t.TrainingId
WHERE ut.Status = 'Completed' AND ut.CertificateExpiryDate IS NOT NULL
ORDER BY ut.CertificateExpiryDate ASC;