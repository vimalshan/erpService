...
docker run -d -p 7136:7136 --name auth-provider -e ASPNETCORE_ENVIRONMENT=Production auth-provider:latest
...
# Test
echo "Testing health endpoint at http://localhost:7136/api/v1/minimal/auth/health"
if command -v curl &> /dev/null; then
    curl -f http://localhost:7136/api/v1/minimal/auth/health || echo "Health check failed. Try http://localhost:7136/swagger/index.html"
else
...
echo "  Health  : http://localhost:7136/api/v1/minimal/auth/health"
